...
# for testing purposes change this to number of frames you want to process
FRAME_LIM = 0

# minimum similarity for object to be re-identified
SIMILARITY_THRESH = 0.78

PATH_TO_DATA = 'street/street_persons_frames'
PATH_TO_OUTPUT = 'street/outputs'
...
filenames = []
# filtered list of those files with confidence > 0.5 and area above 500
# to be done automatically
filenames_list = pd.read_csv("street/street_filenames.csv", header=None).values.tolist()
filenames_list = [i for sub in filenames_list for i in sub]
...
reid_df.to_csv(PATH_TO_OUTPUT + '/street_ReID.csv')
