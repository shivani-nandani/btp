...
def drawBoundingBoxes(imageData, imageOutputPath, inferenceResults, color):
    ...
    for res in inferenceResults:
        ...
        cv2.putText(imageData, label, (left, top - 12),
                    0, 1e-3 * imgHeight, color, thick)
...
def getBoundingBoxes(reid_results_path, detection_results_path, frames_path):
    ...
    for i in range(len(reid_results)):
        ...
        imgcv = cv2.imread(
            frames_path+'/'+reid_results.filename[i].split('_')[0]+'.jpg')
        color = (255, 0, 0)
        results = [
            {
        ...
        drawBoundingBoxes(imgcv, frames_path+'/' +
                          reid_results.filename[i].split('_')[0]+'.jpg', results, color)
